...
export const ApButton: React.FC<IProps> = ({
  ...
  if (outline) {
    ...
    switch (color) {
      case 'primary':
        colorScheme = 'bg-[#056DFF] text-white border border-skin-primary'
        break;
      case 'secondary':
  ...
  const defaultClassName = `${rounded ? rounded : ''} ${btnSize === 'sm' ? 'py-2.4 px-6 text-sm' : 'py-3.5 px-8'} ${btnSize === 'md' && 'py-2.5 px-6 text-sm'}
